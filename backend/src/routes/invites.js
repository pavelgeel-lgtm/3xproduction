...
router.post('/generate', verifyJWT, checkRole(...CAN_INVITE), async (req, res) => {
  const { role, project_id, warehouse_zone } = req.body
  if (!role) return res.status(400).json({ error: 'Missing role' })

  try {
    const token = crypto.randomBytes(24).toString('hex')
    const { rows } = await db.query(
      `INSERT INTO invites (token, role, project_id, warehouse_zone, created_by)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [token, role, project_id || null, warehouse_zone || null, req.user.id]
    )
